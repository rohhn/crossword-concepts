...
class Crossword(object):
    ...
    def legend(self):
        ...
        outStr = ''
        for word in self.current_word_list:
            outStr += '%s (%d,%d) %s: %s\n' % (word.word, word.col, word.row, word.down_across(), word.clue)

        results = []
        pattern = re.compile(r'^([\w-]+)\s*\((\d+),(\d+)\)\s*(down|across):\s*(.+)$', re.IGNORECASE)
        for line in outStr.strip().splitlines():
            match = pattern.search(line)
            if match:
                word, row, col, direction, clue = match.groups()
                results.append({
                    "answer": word,
                    "x": int(row),
                    "y": int(col),
...
class Word(object):
    def __init__(self, word="", clue=None):
